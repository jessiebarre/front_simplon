/**
 ...
 méthode pour travailler sur le DNS. DNS est le Système de Nom de Domaine
 (Domain Name System).

 Le module DNS est documenté ici : https://nodejs.org/docs/latest/api/dns.html
 ...
 connectée à un réseau. Si votre ordinateur à plusieurs cartes réseau
 (wifi, cable, ...), qui sont toutes connectées à des réseaux, alors vous aurez
 ...
 hexadécimal. Par exemple : 2a03:2880:2130:7f20:face:b00c:0:25de pour le
 site Internet de facebook.

 A l'heure actuelle, les réseaux utilisent encore le système d'adresse IPv4.
 Pour connaître votre adresse IPv4 :
 - Sur Windows, sur le terminal on tape : "ipconfig /all" et on s'intéresse au
 champ "IPv4"
 - Sur MAC, sur le terminal on tape : "ifconfig -a" et on s'intéresse au
 dernier champ "inet"
 - Sur Linux, sur le terminal on tape : "ifconfig -a" ou "ip a" et on
 s'intéresse au champ "inet adr"
 ...
 contenant des correspondances entre adresses IPv4 et nom de domaine. Les
 adresses IPv4 de ces ordinateurs font partie de la configuration de vos
 cartes réseau.
 ...
 -> Sur Windows :
 ipconfig /all et voir la rubrique serveurs DNS
 -> Sur Mac :
 networksetup -getdnsservers Wi-Fi ou networksetup -getdnsservers Ethernet
 -> Sur Linux :
 cat /etc/resolv.conf
 **/
...
/**
 ...
 1. Utilisez la méthode .getServers() du module DNS de Node JS pour afficher
 ...
 **/
const myModuleDNS = require("dns");
console.log(myModuleDNS.getServers());
/**
 ...
 **/
myModuleDNS.lookup("www.fnac.com", function (error, addresses) {
